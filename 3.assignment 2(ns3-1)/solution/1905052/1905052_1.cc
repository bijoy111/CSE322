...
NS_LOG_COMPONENT_DEFINE("Wireless high rate static");
...
int main(int argc, char* argv[])
{
    ...
    std::string file1 = "./scratch/graphs1/data1.txt";
    std::string file2 = "./scratch/graphs1/data2.txt";
    std::string file3 = "./scratch/graphs1/data3.txt";
    std::string file4 = "./scratch/graphs1/data4.txt";
    std::string file5 = "./scratch/graphs1/data5.txt";
    std::string file6 = "./scratch/graphs1/data6.txt";
    std::string file7 = "./scratch/graphs1/data7.txt";
    std::string file8 = "./scratch/graphs1/data8.txt";
    ...
    cmd.AddValue ("coverage_area","Static coverage area", coverage_area);
    cmd.Parse(argc, argv);
    ...
    Config::SetDefault ("ns3::RangePropagationLossModel::MaxRange", DoubleValue(coverage_area));
    ...
    pointToPoint.SetDeviceAttribute("DataRate", StringValue("1Mbps")); // bottleneck datarate
    ...
    for (int i = 0; i < total_flow; i++)
    {
        ...
        Ptr<OnOffApplication> sender = DynamicCast<OnOffApplication>(s_App.Get(0)); // see file -> on-off-helper.cc
    ...
    oss << "/NodeList/" << swifiStaNodes.Get(node_per_side - 1)->GetId()
    ...
    */
    ...
    //NS_LOG_UNCOND(sending_packet);
    //NS_LOG_UNCOND(receiving_packet);
    ...
    MyFile7<<coverage_area<<" "<<(total_packet_size*8.0)/(Simulator::Now().GetSeconds())<<std::endl;
    MyFile7.close();

    std::ofstream MyFile8(file8, std::ios_base::app);
    MyFile8<<coverage_area<<" "<<((receiving_packet*1.0)/sending_packet)<<std::endl;
    MyFile8.close();
